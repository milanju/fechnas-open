...
function generateBracket($con, $shuffledParticipantsArray){
	...
	for($i = 0; $i < $bracket_size;$i++){
		if($byeCount > 0){
			...
			insertInTable($con, $bracket, 'spot', 'name', $slot, $shuffledParticipantsArray[$array_pos]);  
			$array_pos++;
			$slot+=2;
	...
	for($i = 0; $i < $bracket_size; $i++) {
		...
		$advance = getSpecificValue($con, $bracket, 'spot', 'name', $opponentSlot);
		
		if ($bye == "*BYE*") {
			updateTableSpecific($con, 'bracket_ro'.$bracket_extension*2, 2, 'spot', 'score', $opponentSlot);
			updateTableSpecific($con, 'bracket_ro'.$bracket_extension, $advance, 'spot', 'name', $opponentSlot/2);
			updateTableSpecific($con, 'bracket_ro'.$bracket_extension*2, 0, 'spot', 'score', $byespot);
...
//function: getEmptySlot returns first empty slot (empty slot = WHERE Name="-") 
function getEmptySlot($con){
...
//function: getValue returns specific Value. Enter <TABLE ID>, <SPOT>. Value=<NAME> WHERE <SPOT>

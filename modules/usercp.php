<div id="user_cp_div" class="content-module">
	...
	if (!(empty($_COOKIE["rememberParticipant"]))) {
		...
		if(getSpecificValue($con, 'data', 'data', 'value', 'bracket') != 'none'){
			echo 'You\'re currently playing in ro'.
			getBracketSize(getUserBracket($con, $_COOKIE["rememberParticipant"])).'.';
		}
		...
		<!--Form for User CP-->
		<button title="Click to edit Name#Code" style="width:190px;" class="button"
									type="button"
		...
		<form action='' method='POST'>
		<input type='submit' name='unsign'
				   value='Unsign from Tournament' class='button'>
		</form>
		<?php } else {?>

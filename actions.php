...
setcookie("rememberUsername", $_COOKIE["rememberUsername"], time()- 365 * 24 * 3600);
...
//<!--Check if Register-Button in register.php has been pressed if true > run register User to Database-->
if(!empty($_POST["register_user"])){
//insertInTable($con, 'users', 'name', 'password', $_POST["register_name"], $_POST["register_pw"]);
...
//<!--Check if Register-Button in header.php has been pressed if true > REDIRECT to register.php-->
...
if (getInformation_Users($con, 'users', 'name', $_POST["login_name"]) === $_POST["login_name"] && getInformation_Users($con, 'users', 'password', $_POST["login_pw"]) === $_POST["login_pw"]) {
	//IF DATA CORRECT
	setcookie("rememberUsername", $_POST["login_name"], time() + 365 * 24 * 3600);
}
else {
...
if(!empty($_POST["unsign"])){

deleteTableElement($con, 'participants', 'name', $_COOKIE["rememberParticipant"]);
...
//<!--Check if RESET button(Participants) has been pressed if true > run resetBracket()-->

if(!empty($_POST["reset_participants"])){
resetTable($con, 'participants');
...
if(!empty($_POST["editParticipant"]) && (!empty($_POST["editCharactercode"]))){
if (!empty($_COOKIE["editBracket"])) {
...
updateTableSpecific($con, 'participants', $participant, 'name', 'name', $_COOKIE["rememberParticipant"]);
$_COOKIE["participant"] = $participant;
$expire = time() + 3600;
setcookie("rememberParticipant", $participant, $expire);
...
//<!--Check if "Report Winner Button" has been pressed. IF YES will update winner.-->
 if(!empty($_POST["ro16submit"])){
$ro8_slot=0;
for($i=1;$i<=16;$i=$i+2){
	$ro8_slot++;
	$j=$i+1;
		if($_POST["ro16submit"]=="$i.W"){
